    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (const auto& pair : myMap) {
            ...
            if (pair.second.size() >= 4) {
                ...
                std::cout << "Alpha of first pixel: " << static_cast<int>(alpha) << std::endl;
            } else {
...
GLint alphaChannel;
glGetTexLevelParameteriv(GL_TEXTURE_2D, 0, GL_TEXTURE_ALPHA_SIZE, &alphaChannel);
std::cout << "Alpha channel size: " << alphaChannel << std::endl;
if (alphaChannel > 0) {
    std::cout << "Texture supports alpha channel" << std::endl;
} else {
    std::cout << "Texture does not support alpha channel" << std::endl;
}
            ...
            if (whole_pixels.size() >= 4) {
                ...
                std::cout << "Alpha of whole pixel: " << static_cast<int>(alpha) << std::endl;
            } else {
                std::cout << "Not enough data for an RGBA pixel" << std::endl;
            }
