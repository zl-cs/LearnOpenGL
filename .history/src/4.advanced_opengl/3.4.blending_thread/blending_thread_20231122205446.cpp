...
void window_rendering(int i,GLint width, GLint height){
    ...
    // }
    Shader shaderend("3.2.blending.vs", "/home/zhang/code/LearnOpenGL/src/4.advanced_opengl/3.4.blending_thread/3.1.window_blending.fs");
    shaderend.use();
    shaderend.setMat4("model", model);
    glReadPixels(0,0,  width, height, GL_RGBA, GL_UNSIGNED_BYTE, pixels.data());
    // write to queue 
    std::unique_lock<std::mutex> lock(mtx);
    QueueElement ele;
