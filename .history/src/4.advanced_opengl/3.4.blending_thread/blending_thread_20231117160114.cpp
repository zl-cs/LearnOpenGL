...
#define IMGSIZE SCR_WIDTH * SCR_HEIGHT * 3 // Adjust this to match your image size

struct mq_msgbuf {
    long mtype;
    unsigned char mtext[IMGSIZE]; // Use unsigned char for binary data
} mbuf;
...
void cube_rendering(int i,GLint width, GLint height, int msgid, struct mq_msgbuf *mbuf);
void floor_rendering(int i,GLint width, GLint height, int msgid, struct mq_msgbuf *mbuf);
void window_rendering(int i,GLint width, GLint height, int msgid, struct mq_msgbuf *mbuf);
...
int main()
{
    ...
    std::cout << "The ID of message queue: "<< msgid << std::endl;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        i += 1;
        thread cube_rendering_thread(cube_rendering,i,viewport[2],viewport[3],msgid,&mbuf);
        thread floor_rendering_thread(floor_rendering,i,viewport[2],viewport[3],msgid,&mbuf);
        thread window_rendering_thread(window_rendering,i,viewport[2],viewport[3],msgid,&mbuf);
...
void cube_rendering(int i,GLint width, GLint height, int msgid, struct mq_msgbuf *mbuf){  //, GLFWwindow* window_main
    ...
    mbuf.mtype = 1;  // mtype应该是正整数，0是非法的
    memset(mbuf.mtext, 0, sizeof(mbuf.mtext));
    mbuf.mtext[0] = pixels.data();
...
void floor_rendering(int i,GLint width, GLint height, int msgid, struct msgbuf *mbuf){
...
void window_rendering(int i,GLint width, GLint height, int msgid, struct msgbuf *mbuf){
