...
std::queue<std::vector<unsigned char>> myQueue;
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        while (true) {
            ...
            std::vector<unsigned char> val = myQueue.front();
            myQueue.pop();
            lock.unlock();
            m += 1;
            std::cout << "Frame: "<< i <<" Consumer got: " << m << " Size of val: " << val.size()<< std::endl;
            // Check if you're rendering to the default frame buffer of the current window
            // GLint framebuffer;
            // glGetIntegerv(GL_FRAMEBUFFER_BINDING, &framebuffer);
            // if (framebuffer == 0) {
            //     std::cout << "Current buffer belong to window!"<< std::endl;
            // }
            // Assuming data is a pointer to your pixel data and width and height are the dimensions of the image
            glDrawPixels(viewport[2],viewport[3], GL_RGBA, GL_UNSIGNED_BYTE, val.data());
            
            if (m == 8) break;
        }

        // Assuming width and height are the dimensions of your window
        std::vector<unsigned char> whole_pixels(viewport[2]*viewport[3]*3); // *4 for RGBA
        glReadPixels(0, 0, viewport[2],viewport[3], GL_RGB, GL_UNSIGNED_BYTE, whole_pixels.data());
...
void cube_rendering(int i,GLint width, GLint height){  //, GLFWwindow* window_main
    ...
    lock.unlock();
    cv.notify_one();
    std::cout << "queue push: cube 2" << std::endl;
...
void floor_rendering(int i,GLint width, GLint height){
    ...
    std::unique_lock<std::mutex> lock(mtx);
    myQueue.push(pixels);
    std::cout << "queue push: floor" << std::endl;
    lock.unlock();
    cv.notify_one();
...
void window_rendering(int i,GLint width, GLint height){
    ...
    for (std::map<float, glm::vec3>::reverse_iterator it = sorted.rbegin(); it != sorted.rend(); ++it)
    {
        ...
        std::unique_lock<std::mutex> lock(mtx);
        myQueue.push(pixels);
        // std::cout << "queue push: window" << z <<std::endl;
        lock.unlock();
        cv.notify_one();
