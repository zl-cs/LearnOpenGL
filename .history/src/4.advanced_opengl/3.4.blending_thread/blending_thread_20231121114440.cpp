...
int main()
{
    ...
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, viewport[2],viewport[3]); // use a single renderbuffer object for both a depth AND stencil buffer.
...
GLint alphaBits;
glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_FRAMEBUFFER_ATTACHMENT_ALPHA_SIZE, &alphaBits);
std::cout << "alphaBits: " << alphaBits << std::endl;
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, viewport[2], viewport[3], 0, GL_RGBA, GL_UNSIGNED_BYTE, myMap[key].data());
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        ...
        write_png_area(whole_pixels, i, viewport[2],viewport[3], "wholetest"+std::to_string(3));

            glBindTexture(GL_TEXTURE_2D, texture2); // use the color attachment texture as the texture of the quad plane
            glDrawArrays(GL_TRIANGLES, 0, 6);
        glReadPixels(0, 0, viewport[2],viewport[3], GL_RGBA, GL_UNSIGNED_BYTE, whole_pixels.data());
        write_png_area(whole_pixels, i, viewport[2],viewport[3], "wholetest"+std::to_string(2));
        ...
        // std::vector<unsigned char> whole_pixels(viewport[2]*viewport[3]*4); // *4 for RGBA
        glReadPixels(0, 0, viewport[2],viewport[3], GL_RGBA, GL_UNSIGNED_BYTE, whole_pixels.data());
        std::cout << "Size of whole_pixels.data: " << whole_pixels.size() << std::endl;
        write_png_area(whole_pixels, i, viewport[2],viewport[3], "whole"+std::to_string(m));
...
void window_rendering(int i,GLint width, GLint height){
    ...
    std::vector<GLubyte> pixels(width * height * 4);
    // glReadPixels(0,0,  width, height, GL_RGBA, GL_UNSIGNED_BYTE, pixels.data());
    // write_png_area(pixels, i, width, height, "windowstest");
    // draw objects
    shader.use();
    glm::mat4 projection = glm::perspective(glm::radians(camera.Zoom), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
    glm::mat4 view = camera.GetViewMatrix();
