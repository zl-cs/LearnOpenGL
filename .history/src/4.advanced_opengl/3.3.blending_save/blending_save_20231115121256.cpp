...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (std::map<float, glm::vec3>::reverse_iterator it = sorted.rbegin(); it != sorted.rend(); ++it)
        {
            ...
            std::fill(pixels.begin(), pixels.end(), 0);
            // glReadPixels(0, 0, viewport[2], viewport[3], GL_RGB, GL_UNSIGNED_BYTE, pixels.data());
            // write_png(pixels, i, viewport, "windows"+std::to_string(x));
            glReadPixels(x, y,  width, height, GL_RGB, GL_UNSIGNED_BYTE, pixels.data());
...
void write_png_area(std::vector<GLubyte>pixels, int i, GLsizei width, GLsizei height, string type)
{
    ...
    int result = stbi_write_png(("output/" + std::to_string(i) + "_"+type+".png").c_str(), width, height, 3, pixels.data(), width * 3);
    if (result == 0) {
        std::cout << "Failed to write PNG:" << std::to_string(i) << std::endl;
    } else {
        std::cout << "Successfully wrote PNG:" << std::to_string(i) <<std::endl;
