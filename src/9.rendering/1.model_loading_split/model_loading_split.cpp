...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::mat4 projection = glm::perspective(glm::radians(camera.Zoom), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f); // 缩放
        ...
        // for (int i = 0; i < 100; ++i) {
        //     std::cout << "pixels[" << i << "] = " << static_cast<int>(pixels[i]) << std::endl;
        // }
        ...
        int result = stbi_write_png(("output/" + std::to_string(i) + ".png").c_str(), viewport[2], viewport[3], 3, pixels.data(), viewport[2] * 3);
        if (result == 0) {
